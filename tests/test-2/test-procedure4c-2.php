...
<?php
	...
	function alpNumRule($alp, $alpb, $alp2, $alp2b, $pn1, $pn2, $pn3, $pn4, $pn5, $pn6, $pn7, $pn8){
		...
		$pnc = 0-$pn5*$pn6+$pn7;
		$alp1 = 2;
		idenCombPowerMultiForm($alp1, $alp2, $alp2b, $pna, $pnb, $pnc); //**harusnya nilai yang direturn, supaya bisa identifikasi bentuknya
		// return $pn;
	}
	...
	function idenCombPowerMultiForm($alp1, $alp2, $alp2b, $pna, $pnb, $pnc){
		if($pna >= 0 && $pnb >= 0 && $pnc >= 0){
			?>
				$${<?php echo $alp1 ?>^{<?php echo $pna ?>}}{<?php echo $alp2 ?>^{<?php echo $pnb ?>}}{<?php echo $alp2b ?>^{<?php echo $pnc ?>}}$$		
			<?php
		}elseif($pna >= 0 && $pnb >= 0 && $pnc < 0){
			$pnc = abs($pnc);
			?>
				$${<?php echo $alp1 ?>^{<?php echo $pna ?>}}{<?php echo $alp2 ?>^{<?php echo $pnb ?>}} \over {<?php echo $alp2b ?>^{<?php echo $pnc ?>}}$$		
			<?php
		}elseif($pna >= 0 && $pnb < 0 && $pnc >= 0){
			$pnb = abs($pnb);
			?>
				$${<?php echo $alp1 ?>^{<?php echo $pna ?>}}{<?php echo $alp2b ?>^{<?php echo $pnc ?>}} \over {<?php echo $alp2 ?>^{<?php echo $pnb ?>}}$$		
			<?php
		}elseif($pna < 0 && $pnb >= 0 && $pnc >= 0){
			$pna = abs($pna);
			?>
				$${<?php echo $alp2 ?>^{<?php echo $pnb ?>}}{<?php echo $alp2b ?>^{<?php echo $pnc ?>}} \over {<?php echo $alp1 ?>^{<?php echo $pna ?>}}$$		
			<?php
		}elseif($pna >= 0 && $pnb < 0 && $pnc < 0){
			$pnb = abs($pnb);
			$pnc = abs($pnc);
			?>
				$${<?php echo $alp1 ?>^{<?php echo $pna ?>}} \over {<?php echo $alp2 ?>^{<?php echo $pnb ?>}}{<?php echo $alp2b ?>^{<?php echo $pnc ?>}}$$		
			<?php
		}elseif($pna < 0 && $pnb >= 0 && $pnc < 0){
			$pna = abs($pna);
			$pnc = abs($pnc);
			?>
				$${<?php echo $alp2 ?>^{<?php echo $pnb ?>}} \over {<?php echo $alp1 ?>^{<?php echo $pna ?>}}{<?php echo $alp2b ?>^{<?php echo $pnc ?>}}$$		
			<?php
		}elseif($pna < 0 && $pnb < 0 && $pnc >= 0){
			$pna = abs($pna);
			$pnb = abs($pnb);
			?>
				$${<?php echo $alp2b ?>^{<?php echo $pnc ?>}} \over {<?php echo $alp1 ?>^{<?php echo $pna ?>}}{<?php echo $alp2 ?>^{<?php echo $pnb ?>}}$$		
			<?php
		}else{
			$pna = abs($pna);
			$pnb = abs($pnb);
			$pnc = abs($pnc);
			?>
				$$1 \over {<?php echo $alp1 ?>^{<?php echo $pna ?>}}{<?php echo $alp2 ?>^{<?php echo $pnb ?>}}{<?php echo $alp2b ?>^{<?php echo $pnc ?>}}$$		
			<?php
		}
		/*?>
			$${<?php echo $alp1 ?>^{<?php echo $pna ?>}}{<?php echo $alp2 ?>^{<?php echo $pnb ?>}}{<?php echo $alp2b ?>^{<?php echo $pnc ?>}}$$		
	...
	function idenCombPowerMultiForm2($alp1, $alp1b, $alp2, $alp2b, $pna, $pnb, $pnc, $pnd){
		if($pnb >= 0 && $pnd >= 0){
			?>
			...
			$pnd = abs($pnd);
			?>
				$${<?php echo $alp1 ?>^{<?php echo $pna ?>}}{<?php echo $alp2 ?>^{<?php echo $pnb ?>}} \over {<?php echo $alp1b ?>^{<?php echo $pnc ?>}}{<?php echo $alp2b ?>^{<?php echo $pnd ?>}}$$		
			<?php
		}elseif($pnb < 0 && $pnd >= 0){
			$pnb = abs($pnb);
			?>
				$${<?php echo $alp1b ?>^{<?php echo $pnc ?>}}{<?php echo $alp2b ?>^{<?php echo $pnd ?>}} \over {<?php echo $alp1 ?>^{<?php echo $pna ?>}}{<?php echo $alp2 ?>^{<?php echo $pnb ?>}}$$		
			<?php
		}else{
			$pnb = abs($pnb);
			$pnd = abs($pnd);
			?>
				$$1 \over {<?php echo $alp1 ?>^{<?php echo $pna ?>}}{<?php echo $alp2 ?>^{<?php echo $pnb ?>}}{<?php echo $alp1b ?>^{<?php echo $pnc ?>}}{<?php echo $alp2b ?>^{<?php echo $pnd ?>}}$$		
			<?php
		}
	}
?>
